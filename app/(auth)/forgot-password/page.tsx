...
import { Mail, KeyRound, Loader2, ChevronLeft, CheckCircle2 } from "lucide-react";
...
export default function ForgotPasswordPage() {
  ...
  const handleResetSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If the code is "incorrect", Clerk will tell us here
      ...
      <div className="w-full max-w-md bg-white shadow-2xl flex flex-col min-h-screen md:min-h-0 md:h-[850px] md:my-auto md:rounded-[2.5rem] overflow-hidden relative">
        ...
        <div className="bg-[#0056d2] pt-8 pb-16 relative">
          <div className="px-6 flex items-center relative z-10">
            <Link href="/sign-in" className="text-white mr-4"><ChevronLeft /></Link>
        ...
        <div className="px-8 pt-4 flex flex-col items-center flex-1">
          <div className={`w-32 h-32 rounded-full flex items-center justify-center mb-6 border-4 border-white shadow-inner ${successfulId ? 'bg-green-50' : 'bg-blue-50'}`}>
            {successfulId ? <CheckCircle2 className="w-16 h-16 text-green-500" /> : <KeyRound className="w-16 h-16 text-blue-500" />}
          </div>

          <h1 className="text-2xl font-black text-slate-800">{successfulId ? "Verify Code" : "Reset Password"}</h1>
            ...
            <form onSubmit={handleResetRequest} className="w-full mt-8 space-y-4">
              <div className="relative">
                ...
                <input 
                  ...
                  className="w-full pl-12 pr-4 py-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:border-blue-500"
                />
              </div>
              <button type="submit" disabled={loading} className="w-full bg-[#0056d2] text-white font-black py-4 rounded-2xl shadow-lg">
                {loading ? <Loader2 className="animate-spin mx-auto" /> : "SEND RESET CODE"}
              </button>
            </form>
          ) : (
            <form onSubmit={handleResetSubmit} className="w-full mt-8 space-y-4">
              <input 
                type="text" placeholder="6-digit code" required value={code} onChange={(e) => setCode(e.target.value)}
                className="w-full py-4 bg-slate-50 border-2 border-blue-500 rounded-2xl text-center text-2xl font-black tracking-widest outline-none"
              />
              <div className="relative">
              ...
              <button type="submit" disabled={loading} className="w-full bg-[#2d8a4e] text-white font-black py-4 rounded-2xl shadow-lg">
                {loading ? <Loader2 className="animate-spin mx-auto" /> : "UPDATE PASSWORD"}
              </button>
              <button type="button" onClick={() => setSuccessfulId(false)} className="w-full text-slate-400 text-xs font-bold uppercase tracking-widest mt-2">
                Resend Code
              </button>
            </form>
          )}
